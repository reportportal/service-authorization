...
/**
 * Third-party OAuth providers enumeration
 *
...
public enum OAuthProvider {
	GITHUB("github",
		...
		@Override
		void applyDefaults(OAuth2LoginDetails details) {
	...
	OAuthProvider(String name, String button) {
		this.name = name;
		this.button = button;
	}
	 ...
	 */
	abstract void applyDefaults(OAuth2LoginDetails details);
	...
	public static Optional<OAuthProvider> findByName(String name) {
		return Arrays.stream(values()).filter(provider -> provider.getName().equals(name)).findAny();
	}
}
