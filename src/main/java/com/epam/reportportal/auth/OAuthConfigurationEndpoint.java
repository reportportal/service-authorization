...
public class OAuthConfigurationEndpoint {
    ...
    public Map<String, OAuthDetailsResource> updateOAuthSettings(@PathVariable String profileId,
            ...
            @RequestBody @Validated OAuthDetailsResource oauthDetails) {
    ...
    public Map<String, OAuthDetailsResource> getOAuthSettings(@PathVariable String profileId) {
        ...
        return Optional
                .ofNullable(settings.getoAuth2LoginDetails())
                .map(details -> details.entrySet().stream()
                        .collect(Collectors
                                .toMap(Map.Entry::getKey, e -> OAuthDetailsConverters.TO_RESOURCE.apply(e.getValue()))))
