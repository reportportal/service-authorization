...
public class SamlServiceProviderConfiguration {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(
      SamlServiceProviderConfiguration.class);
  ...
  private LocalServiceProviderConfiguration serviceProviderConfiguration() {
    LocalServiceProviderConfiguration serviceProviderConfiguration =
        new LocalServiceProviderConfigurationDelegate(integrationTypeRepository);
  ...
  private List<ExternalIdentityProviderConfiguration> providers() {
    Optional<IntegrationType> saml = integrationTypeRepository.findByName(
  ...
  private SimpleKey getActiveKey() {
    ...
    if (signedRequests) {
      X509Certificate certificate = CertificationUtil.getCertificateByName(keyAlias, keyStore,
          keyStorePassword);
      PrivateKey privateKey = CertificationUtil.getPrivateKey(keyAlias, keyPassword, keyStore,
          keyStorePassword);
  ...
  public class LocalServiceProviderConfigurationDelegate extends LocalServiceProviderConfiguration {
    ...
    public String getBasePath() {
      return integrationTypeRepository.findByName(AuthIntegrationType.SAML.getName())
          ...
          .orElseGet(() -> ofNullable(basePath).orElseGet(
              () -> ofNullable(requestProvider.get()).map(RequestUtil::getRequestBasePath)
                  .orElse("/")));
    }
  }
