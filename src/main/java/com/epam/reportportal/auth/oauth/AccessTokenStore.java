...
/**
 * Implementation of custom token store. It is used to store unexpired api token in the database. It
 * is a dirty hack to use it with test frameworks. It just stores an {@link AccessTokenStore} and
...
public class AccessTokenStore implements TokenStore {
  ...
  public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
    ...
    tokenEntity.setRefreshToken(
        null == token.getRefreshToken() ? null : token.getRefreshToken().getValue());
  ...
  public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
    StoredAccessToken token = oAuth2AccessTokenRepository.findByAuthenticationId(
