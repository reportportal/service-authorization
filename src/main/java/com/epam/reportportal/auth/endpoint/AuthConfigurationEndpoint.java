...
public class AuthConfigurationEndpoint {
	...
	public LdapConfig updateLdapSettings(@RequestBody @Valid LdapConfig ldapConfig) {
		...
		return repository.findLdap(true).orElseThrow(() -> new ReportPortalException(ErrorType.OAUTH_INTEGRATION_NOT_FOUND, ldapConfig.getId()));
	}
	...
	public ActiveDirectoryConfig updateADSettings(@RequestBody @Validated ActiveDirectoryConfig adConfig) {
		...
		return repository.findActiveDirectory(true).orElseThrow(() -> new ReportPortalException(ErrorType.OAUTH_INTEGRATION_NOT_FOUND, adConfig.getId()));
	}
