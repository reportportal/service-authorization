...
public class AuthConfigService {
  ...
  private static BaseOAuth2ProtectedResourceDetails getOauth2ProtectedResourceDetails(
    ...
    return ofNullable(oAuthRegistrationResource.getAuthGrantType()).map(grantType -> {
      Optional<String> authorizationUri = ofNullable(
          oAuthRegistrationResource.getAuthorizationUri());
      switch (grantType) {
   ...
   */
  public OAuth2RestOperations getRestTemplate(String name,
      OAuth2ClientContext oauth2ClientContext) {
    return newProxy(OAuth2RestOperations.class, (proxy, method, args) -> {
      try {
        return method.invoke(new OAuth2RestTemplate(loadResourceDetails(name), oauth2ClientContext),
            args);
  ...
  /**
   * Loads {@link OAuthRegistration} from database and converts it to the
   * {@link OAuthRegistrationResource}
   *
  ...
  public Optional<OAuthRegistrationResource> loadOAuthRegistration(String id) {
    return clientRegistrationRepository.findOAuthRegistrationById(id)
  ...
  public OAuth2ProtectedResourceDetails loadResourceDetails(String name) {
    return loadOAuthRegistration(name).map(RESOURCE_DETAILS_CONVERTER)
        .orElseThrow(() -> noAuthDetailsException(name));
  }
