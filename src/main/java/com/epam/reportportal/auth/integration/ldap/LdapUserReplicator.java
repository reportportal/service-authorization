...
public class LdapUserReplicator extends AbstractUserReplicator {
   ...
   */
  @Transactional
  public User replicateUser(String name, DirContextOperations ctx,
      Map<String, String> syncAttributes) {
    String emailAttribute = ofNullable(
        syncAttributes.get(LdapParameter.EMAIL_ATTRIBUTE.getParameterName())).orElseThrow(
        () -> new UserSynchronizationException(
    ...
    if (userOptional.isEmpty()) {
      ...
      ofNullable(syncAttributes.get(LdapParameter.FULL_NAME_ATTRIBUTE.getParameterName())).filter(
              StringUtils::isNotBlank)
          .flatMap(it -> ofNullable(ctx.getStringAttribute(it)))
          .ifPresent(newUser::setFullName);

      ofNullable(syncAttributes.get(LdapParameter.PHOTO_ATTRIBUTE.getParameterName())).filter(
              StringUtils::isNotBlank)
      ...
      throw new UserSynchronizationException(
          "User with login '" + userOptional.get().getLogin() + "' already exists");
    }
